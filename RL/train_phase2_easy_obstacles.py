#!/usr/bin/env python3
"""Phase 2: Open + easy obstacles (challenge types 4 and 3). Loads Phase 1 by default."""
import argparse
import random
import sys
...
from RL.rewards import phase2_reward
from RL.callbacks import EvalLoggingCallbackFromVecEnv

DEFAULT_PREVIOUS = "swarm/submission_template/phase1_open_only.zip"
PHASE2_TYPES = [4, 3]
...
def main():
    ...
    parser.add_argument("--checkpoint-dir", type=str, default="logs/phase2_checkpoints")
    parser.add_argument("--tensorboard-log", type=str, default="logs/phase2")
    ...
    def task_factory():
        t = random.choice(PHASE2_TYPES)
        return random_task_with_type(sim_dt=SIM_DT, seed=random.randint(0, 2**31 - 1), challenge_type=t)

    def eval_task_factory(seed: int):
        t = PHASE2_TYPES[seed % len(PHASE2_TYPES)]
        return random_task_with_type(sim_dt=SIM_DT, seed=seed, challenge_type=t)

    def make_wrapped():
        inner = ResettableTaskWrapper(task_factory, make_env_no_gui)
        return RewardWrapper(inner, phase2_reward)
    ...
    callbacks = [
        CheckpointCallback(save_freq=args.save_freq, save_path=args.checkpoint_dir, name_prefix="phase2"),
    ...
    model.save(str(out_dir / "phase2_easy_obstacles.zip"))
    env.close()

    print(f"Saved: {out_dir / 'phase2_easy_obstacles.zip'}")
    print("Next: python RL/train_phase3_full_mix.py --timesteps 200000")
