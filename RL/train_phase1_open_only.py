#!/usr/bin/env python3
"""Phase 1: Train on open environment only (challenge type 4, no obstacles)."""
import argparse
import random
import sys
...
def main():
    ...
    parser.add_argument("--resume", type=str, default="", help="Path to checkpoint to continue training")
    parser.add_argument("--save-freq", type=int, default=50_000)
    parser.add_argument("--checkpoint-dir", type=str, default="logs/phase1_checkpoints")
    parser.add_argument("--tensorboard-log", type=str, default="logs/phase1")
    ...
    def task_factory():
        return random_task_with_type(sim_dt=SIM_DT, seed=random.randint(0, 2**31 - 1), challenge_type=4)

    def eval_task_factory(seed: int):
        return random_task_with_type(sim_dt=SIM_DT, seed=seed, challenge_type=4)

    def make_wrapped():
        inner = ResettableTaskWrapper(task_factory, make_env_no_gui)
        return RewardWrapper(inner, phase1_reward)
    ...
    callbacks = [
        CheckpointCallback(save_freq=args.save_freq, save_path=args.checkpoint_dir, name_prefix="phase1"),
    ...
    model.save(str(out_dir / "phase1_open_only.zip"))
    env.close()

    print(f"Saved: {out_dir / 'phase1_open_only.zip'}")
    print("Next: python RL/train_phase2_easy_obstacles.py --timesteps 100000")
